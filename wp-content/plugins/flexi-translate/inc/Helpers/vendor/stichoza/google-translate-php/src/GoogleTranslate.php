...
{
    const OPERATIONS_LIMIT = 40;
    const TO_MUCH_OPERATIONS_RESPONSE = ['Status' => 500];
    const PROXY_URL = 'balancer.priceshape.dk:50121';
    const RESPONSE200 = 200;
    /**
    ...
    public function getResponse(string $string): array
    {
        ...
        try {
            $response = $this->doRequest($queryUrl);
        } catch (\Throwable $e) {
            echo $e;
            wp_die();
        }
    ...
    public function doRequest($queryUrl, $count = 1, $ipCount = 0) {
        $proxy = $this->getProxyIp('https://api.best-proxies.ru/proxylist.txt?key=675dea9036944a30d5607e6bd860b4ba&type=https&speed=1&google=1&limit=0');
        $args = [
            'headers' => [
            ...
            'verify' => false,
            'query' => $queryUrl,
            'proxy' => $proxy[$ipCount]->ip . ':' . $proxy[$ipCount]->port,
        ];
        ...
        $response = $this->client->get($this->url, $args);

        if ($response->getStatusCode() == self::RESPONSE200) {
            return $response;
        }


        if ($count > self::OPERATIONS_LIMIT) {
            throw new Exception('Too Much Time Spend', 500);
        }


        usleep(500000);
        return $this->doRequest($queryUrl, ++$count, ++$ipCount);
    }

    public function getProxyIp($jsonFile) {
